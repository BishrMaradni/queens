...
#
"""Test-module for Gaussian Random Field."""
...
from queens.parameters.random_fields import GPRRandomField
...
def fixture_parameters(pre_processor):
    ...
    parameters = Parameters(field_1=field_1)
    return parameters
...
def test_grad_joint_logpdf(parameters):
    """Test *grad_joint_logpdf* method."""
    samples = np.ones(shape=(1, 8))
    grad_logpdf = parameters.grad_joint_logpdf(samples)
    pytest.approx(grad_logpdf, -np.ones(shape=(2, 8)))


def test_sample_as_dict(parameters):
    """Test *sample_as_dict* method."""
    sample = np.ones(shape=(1, 8))
...
def fixture_pre_processor():
    ...
    class PreProcessor:
        ...
        def __init__(self):
            """Initialize."""
            g = np.meshgrid(np.linspace(0, 1, 2), np.linspace(0, 1, 2), np.linspace(0, 1, 2))
            grid = np.stack(
                (g[0].reshape(-1, 1), g[1].reshape(-1, 1), g[2].reshape(-1, 1)), axis=1
            ).squeeze()
            key_list_0 = []
...
def test_from_config_create_parameters(parameters):
    ...
    assert parameters.num_parameters == 8
    ...
    assert parameters.names == ["field_1"]
    rf_1 = parameters.dict["field_1"]
    assert rf_1.corr_length == 0.3

    sample_1 = np.ones(shape=(3, 8))
    ...
    sample_joint = sample_1
    latent_grad = parameters.latent_grad(sample_joint)
    pytest.approx(latent_grad, grad_field_1)
